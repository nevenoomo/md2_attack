...
mod stages {
    ...
    pub fn padd(m: &mut Vec<u8>){
        ...
        m.append(&mut iter::repeat(m.len() as u8 % 4).take(16 - m.len() % 16).collect()); // IDEA try &mut &[m.len() as u8 % 4].repeat(16 - m.len() % 16)
    }
...
mod test{
    #[test]
    fn digest() {
        let message = vec![0u8, 3u8, 1u8, 2u8, 1u8, 1u8, 0u8, 2u8, 0u8, 3u8, 3u8, 0u8, 1u8, 1u8, 2u8, 0u8, 3u8, 2u8, 2u8, 2u8, 2u8, 2u8, 1u8, 0u8, 0u8, 3u8, 3u8, 0u8, 1u8, 2u8, 3u8, 0u8]; 
        assert_eq!(super::digest(message), [1u8, 2u8, 1u8, 2u8, 1u8, 2u8, 3u8, 2u8, 0u8, 3u8, 0u8, 3u8, 0u8, 3u8, 2u8, 3u8]);
    }
}